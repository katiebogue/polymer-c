...
4. Load the .mat files containing polymer-c outputs in struct format. You can aquire these by:
   ...
   2. Making .mat files from prior `polymer-c` runs via the `makeLookupMat()` function; See `makeLookupMat.m` for complete documentation
      1. former polymer-c runs are stored in `Data/polymer-c_data/` (this is not a source controlled folder)
      2. To figure out the settings of a particular `polymer-c` run, see the `out_*.txt` output log file generated by the HPC3 run, as this will have verbose output stating the settings
         1. Note: for some parameters, they are listed twice in this file -- if the two listings differ, the second listing is correct
